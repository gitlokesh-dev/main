...
#   Multiple servers: -LicenseServerFQDN "server1.dom.com;server2.dom.com"
#
# OUTPUT
#   C:\Scripts\RDL\Output\RDSLicenseMonitoring_<ServerName>_<timestamp>.html  (single)
#   C:\Scripts\RDL\Output\RDSLicenseMonitoring_Combined_<timestamp>.html      (multi)
#
# CHANGE LOG
# ----------
#  V1.4  (2026-05-27)  STRUCTURAL REWRITE -- performance + reliability
#        - Script wrapped in top-level try/catch; never exits without cleanup
#        - Parallel WMI/CIM queries via RunspacePool: all servers queried
#          simultaneously so total run time = slowest single server, not sum
#        - CimSession always disposed in finally block (no session leaks)
#        - Removed duplicate $script: counter bug in filter scriptblocks;
#          counters are now local variables accumulated in a foreach loop
#        - Output folder created once at startup, not inside each report function
#        - Build-CombinedReport and Save-SingleServerReport wrapped fully in
#          try/catch with structured error messages
#        - Dead comment block ("EMBEDDED HTML TEMPLATE") removed
#        - EscapeJson made more robust (handles null input without branch)
#        - $ErrorActionPreference set to Stop inside functions; Continue at
#          script level so HPSA still sees partial output on soft failures
#        - Version and threshold constants grouped in dedicated CONFIG region
#        - All regions labelled with #region / #endregion for IDE folding
#
#  V1.3  (2026-05-27)  REFACTOR -- Unified HTML template (single Get-UnifiedTemplate)
#  V1.2  (2026-05-27)  BUG FIX  -- Expired key pack exclusion (ExpirationDate / KeyPackType=6)
...
#region CONFIG
$ErrorActionPreference = "Continue"   # script level: let HPSA see all output
...
# ----------------------------------------------------------------------------
# EscapeJson  --  makes a string safe for embedding in a JSON string literal
# ----------------------------------------------------------------------------
function EscapeJson {
    param([string]$s)
...
# ----------------------------------------------------------------------------
# Get-Compliance  --  maps a usage % to a compliance string
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# ConvertFrom-WmiExpiry  --  parses a WMI datetime string; returns $null if
# the pack never expires (epoch / year <= 1970) or the string is empty.
# Returns [datetime] in UTC if a real expiry date is found.
# ----------------------------------------------------------------------------
function ConvertFrom-WmiExpiry {
    param([string]$Raw)
    if ([string]::IsNullOrWhiteSpace($Raw)) { return $null }
    try {
        $datePart = $Raw.Substring(0, 14)   # yyyyMMddHHmmss
        $dt = [datetime]::ParseExact(
            $datePart, 'yyyyMMddHHmmss',
            [System.Globalization.CultureInfo]::InvariantCulture,
            [System.Globalization.DateTimeStyles]::AssumeUniversal)
        if ($dt.Year -le 1970) { return $null }   # WMI "Never" sentinel
        return $dt.ToUniversalTime()
    } catch {
        return $null   # unparseable = treat as never-expiring
    }
}
#endregion HELPERS
...
# ----------------------------------------------------------------------------
# Get-UnifiedTemplate
# One self-contained HTML page for both single-server and multi-server runs.
# The JavaScript engine detects which variable was injected at runtime:
#   window.REPORT_DATA   -> single server  (injected by Save-Report)
#   window.COMBINED_DATA -> multiple servers (injected by Save-Report)
# Header, KPI cards, overview banner and table all adapt automatically.
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# Save-Report  --  single entry point for both single and multi-server reports.
# Determines mode from $Results count, builds the correct JSON block, writes HTML.
# ----------------------------------------------------------------------------
function Save-Report {
    ...
    if ($Results.Count -eq 0) {
        Write-Log "Save-Report: no results to write -- skipping report." "WARN"
        return
    }
    ...
    try {
        $HtmlTemplate = Get-UnifiedTemplate

        # ---- Single server --------------------------------------------------
        if ($Results.Count -eq 1) {
            $r            = $Results[0]
            $SafeName     = $r.Server -replace '[^a-zA-Z0-9\-\.]', '_'
            $OutputFile   = $OutputDir + "RDSLicenseMonitoring_${SafeName}_${cDT}.html"
            ...
            $errJsonItems = @($r.Errors | ForEach-Object { '"' + (EscapeJson $_) + '"' })
...
"@
            $HtmlTemplate.Replace('</body>', $JsonBlock + "`n</body>") |
        ...
        # ---- Multiple servers -----------------------------------------------
            ...
            $OutputFile = $OutputDir + "RDSLicenseMonitoring_Combined_${cDT}.html"
...
"@
        ...
        Write-Log "FATAL: Report file could not be saved -- $($_.Exception.Message)" "ERROR"
...
# ----------------------------------------------------------------------------
# Get-KeyPacks  --  queries Win32_TSLicenseKeyPack from one server.
# Tries WMI (faster) first; falls back to CIM over DCOM on failure.
# Returns an array of objects, or empty array on complete failure.
# The CimSession is always disposed in a finally block.
# ----------------------------------------------------------------------------
function Get-KeyPacks {
    ...
    try {
        ...
        Write-Log "  WMI query OK -- $($packs.Count) key pack(s) returned" "SUCCESS"
        return $packs
    } catch {
    ...
    try {
        ...
        Write-Log "  CIM query OK -- $($packs.Count) key pack(s) returned" "SUCCESS"
        return $packs
    } catch {
        $msg = "License server [$Server] unreachable via WMI and CIM: $($_.Exception.Message)"
        $ErrorLog.Add($msg)
        Write-Log "  $msg" "ERROR"
        return @()
    } finally {
...
# ----------------------------------------------------------------------------
# Get-OSVersion  --  retrieves the OS caption for the license server.
# Tries WMI first; falls back to CIM.  Returns "N/A" on failure.
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# Invoke-LicenseServerReport  --  full data collection for one server.
# Returns a PSCustomObject with all KPI values and an Errors list.
# Never throws -- all failures are captured in the Errors list.
# ----------------------------------------------------------------------------
function Invoke-LicenseServerReport {
    param([Parameter(Mandatory)][string]$Server)

    Write-Log "--- [$Server] Starting data collection ---"
    ...
    # ---- 2. Query OS version (independent of key pack success) -------------
    $OSVersion = Get-OSVersion -Server $Server -ErrorLog $ErrorLog

    # ---- 3. Filter and calculate CAL totals --------------------------------
    $Installed          = 0
    $Issued             = 0
    $Available          = 0
    $UnlimitedCount     = 0
    ...
    try {
        ...
        foreach ($pack in $KeyPacks) {
            ...
            # Step 1 -- skip unlimited / built-in packs
            ...
            # Step 2 -- skip explicitly expired (KeyPackType = 6)
            if ($pack.KeyPackType -eq 6) {
                $ExpiredCount++
                Write-Log "  [SKIP-EXPIRED-TYPE] $($pack.Description)" "WARN"
                continue
            }

            # Step 3 -- skip expired by date
            ...
            # Step 4 -- deduplicate (Description | Version | Total | Issued)
        ...
        # Audit log -- one line per active pack
        foreach ($pack in $ActivePacks) {
            $expiryStr = if ([string]::IsNullOrWhiteSpace($pack.ExpirationDate)) { "Never" } `
                         else {
            ...
            Write-Log ("  [ACTIVE] {0} | Total={1} | Issued={2} | Available={3} | Expiry={4}" -f `
                $pack.Description, [int64]$pack.TotalLicenses,
                [int64]$pack.IssuedLicenses, [int64]$pack.AvailableLicenses,
                $expiryStr) "SUCCESS"
        }

        # Summary log
        Write-Log ("  Filter summary: Total={0} | Unlimited={1} | Expired={2} | Duplicate={3} | Active={4}" -f `
            $KeyPacks.Count, $UnlimitedCount, $ExpiredCount, $DuplicateCount, $ActivePacks.Count) "INFO"
        Write-Log ("  CAL totals: Installed={0} | Issued={1} | Available={2}" -f `
            $Installed, $Issued, $Available) "SUCCESS"

    } catch {
        $msg = "CAL calculation failed for [$Server]: $($_.Exception.Message)"
        $ErrorLog.Add($msg)
        Write-Log "  $msg" "ERROR"
    ...
    Write-Log ("  Result: {0}/{1} CALs ({2}%) => {3}" -f `
        $Issued, $Installed, $UsagePct, $Compliance)

    if ($ErrorLog.Count -gt 0) {
        Write-Log "  $($ErrorLog.Count) issue(s) logged for [$Server]" "WARN"
    }
    Write-Log "--- [$Server] Complete: $Compliance ---"
...
# ============================================================================
# MAIN
# Parses the server list, queries all servers, saves the report.
# Wrapped in try/catch so HPSA always receives a clean exit code.
# ============================================================================
try {
    Write-Log "================================================================="
    Write-Log "  RDS License Usage Monitoring | Citrix Workspace Automation Suite"
    Write-Log "  $ScriptVersion  |  Thresholds: Warn=$WarningThresholdPct%  Crit=$CriticalThresholdPct%"
    Write-Log "================================================================="
    ...
    if ($ServerList.Count -eq 0) {
        Write-Log "FATAL: LicenseServerFQDN is empty or contains no valid names." "ERROR"
        exit 3
    }
    Write-Log "$($ServerList.Count) server(s) to process: $($ServerList -join ', ')"
    ...
    try {
        ...
        Write-Log "FATAL: Cannot create output folder '$OutputDir': $($_.Exception.Message)" "ERROR"
        exit 3
    }

    # ---- Query all servers -------------------------------------------------
    # Single server: direct call.
    # Multiple servers: parallel RunspacePool so all servers are queried
    # simultaneously -- total run time = slowest server, not sum of all.
    ...
    if ($ServerList.Count -eq 1) {
        # Single -- no overhead of runspace machinery
        $result = Invoke-LicenseServerReport -Server $ServerList[0]
        if ($null -ne $result) { $ServerResults.Add($result) }
        else                   { $FailedServers.Add($ServerList[0]) }

    } else {
        # Multiple -- parallel execution via RunspacePool
        Write-Log "Starting parallel queries (RunspacePool, max $($ServerList.Count) threads)..." "INFO"
        ...
        # Capture script-level variables needed inside each runspace
        $scriptBody = $function:Invoke-LicenseServerReport.ToString()
        $getKpBody  = $function:Get-KeyPacks.ToString()
        $getOsBody  = $function:Get-OSVersion.ToString()
        $wmiExpBody = $function:ConvertFrom-WmiExpiry.ToString()
        $logBody    = $function:Write-Log.ToString()
        $compBody   = $function:Get-Compliance.ToString()
        ...
        foreach ($srv in $ServerList) {
            ...
            [void]$ps.AddScript({
                param($Server, $Warn, $Crit,
                      $fnInvoke, $fnGetKp, $fnGetOs, $fnWmiExp, $fnLog, $fnComp)
                # Recreate functions inside the runspace
                Invoke-Expression "function Write-Log { $fnLog }"
                Invoke-Expression "function Get-Compliance { $fnComp }"
                Invoke-Expression "function ConvertFrom-WmiExpiry { $fnWmiExp }"
                Invoke-Expression "function Get-KeyPacks { $fnGetKp }"
                Invoke-Expression "function Get-OSVersion { $fnGetOs }"
                Invoke-Expression "function Invoke-LicenseServerReport { $fnInvoke }"
                $global:WarningThresholdPct  = $Warn
                $global:CriticalThresholdPct = $Crit
                Invoke-LicenseServerReport -Server $Server
            }).AddParameters(@{
                Server   = $srv
                Warn     = $WarningThresholdPct
                Crit     = $CriticalThresholdPct
                fnInvoke = $scriptBody
                fnGetKp  = $getKpBody
                fnGetOs  = $getOsBody
                fnWmiExp = $wmiExpBody
        ...
        foreach ($job in $Jobs) {
            try {
                ...
                if ($job.PS.HadErrors) {
                    $job.PS.Streams.Error | ForEach-Object {
                        Write-Log "  RunspaceError [$($job.Server)]: $_" "WARN"
                    }
                }
                ...
                $FailedServers.Add($job.Server)
                Write-Log "Job failed for [$($job.Server)]: $($_.Exception.Message)" "ERROR"
            } finally {
                $job.PS.Dispose()
            }
        }
