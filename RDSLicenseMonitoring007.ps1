...
#region PARAMETERS
param(
    # One server FQDN, or several separated by semicolons and/or commas
    # (e.g. "srv1;srv2;srv3" or "srv1,srv2,srv3"). RECOMMENDED for HPSA/Camunda:
    # pass this Base64-encoded instead -- it's auto-detected and decoded, and
    # completely avoids command-line quoting problems since Base64 text has no
    # ';', ',', or '"' characters for an unquoted/mis-quoted command build to
    # mangle. (Same convention already used for credential injection elsewhere
    # in this suite.)
...
# HTML-encodes a string for safe embedding directly into the rendered report markup
# (server names, OS versions, error text, etc.).
function HtmlEncode {
    param([string]$s)
    ...
    try {
        ...
        Write-Log "HtmlEncode failed for input value -- returning empty string." "WARN"
        return ""
    }
}

# If the incoming value is valid Base64, decode and return the underlying text;
# otherwise return the original string untouched. This lets HPSA/Camunda inject
# the server list Base64-encoded (same convention already used for credentials
# in other scripts in this suite) so that no ';' or '"' ever has to survive an
# unquoted/mis-quoted command-line build -- Base64 text is just A-Z a-z 0-9 + / =,
# none of which are special to PowerShell or cmd.exe.
function Resolve-PossibleBase64 {
    param([string]$Value)
    if ([string]::IsNullOrWhiteSpace($Value)) { return $Value }
    $trimmed = $Value.Trim()
    # Cheap pre-check: Base64 alphabet only, length a multiple of 4.
    if ($trimmed -notmatch '^[A-Za-z0-9+/]+={0,2}$' -or ($trimmed.Length % 4) -ne 0) {
        return $Value
    }
    try {
        $bytes   = [System.Convert]::FromBase64String($trimmed)
        $decoded = [System.Text.Encoding]::UTF8.GetString($bytes)
        # A decoded server list should look like plain hostnames/FQDNs separated
        # by ';' or ',' -- reject anything with control characters, which means
        # this wasn't actually Base64-encoded text and just happened to match
        # the alphabet (e.g. a plain single hostname with no separators).
        if ($decoded -match '[^\x20-\x7E]') { return $Value }
        Write-Log "  Detected Base64-encoded server list -- decoded." "INFO"
        return $decoded
    } catch {
        return $Value
    }
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ---- Header (kept intentionally simple/lightweight) --------------- */
        .rpt-hdr {
            background: var(--brand); color: #fff; padding: 10px 28px 8px;
            display: flex; align-items: baseline; justify-content: space-between; gap: 16px; flex-wrap: wrap;
        }
        .hdr-title { font-size: 1.12rem; font-weight: 800; letter-spacing: -.3px; }
        ...
        .sec-hdr-l          { display: flex; align-items: center; gap: 9px; font-weight: 700; font-size: .9rem; }
        ...
        thead th  {
            background: var(--brand-ultra); color: var(--brand-dark); padding: 12px 14px;
...
<div class="rpt-hdr">
    ...
    <div class="hdr-gen">&#128336; Generated: <span id="gen-date">{{GEN_DATE}}</span></div>
</div>
...
<div class="rpt-body">
    ...
    <div class="sec" id="s-kp">
        <div class="sec-hdr" onclick="tog('kp')">
            <span class="sec-hdr-l" id="sec-hdr-label">{{SEC_HDR_LABEL}}</span>
    ...
    <div class="rpt-ftr" id="rpt-ftr">{{FOOTER_HTML}}</div>
...
function Save-Report {
    ...
    try {
        ...
        # Everything below is built here in PowerShell and dropped straight
        # into the template's {{PLACEHOLDER}} tokens. There is no client-side
        # JS/JSON step left -- the saved file (and the stdout copy between
        # the HPSA markers) is a single, fully self-contained HTML document
        # the instant it's written, whether it covers 1 server or many.
        # This is what fixes the "separated output" issue: nothing has to
        # execute in a browser for the data to be visible.

        # -- Table rows -- one <tr> per server, sorted by usage desc (works for a single row too) --
        $SortedResults = @($Results | Sort-Object -Property UsagePct -Descending)
        $RowsHtmlParts = @(foreach ($r in $SortedResults) {
            $col = if ($r.UsagePct -ge $CriticalThresholdPct) { '#BF0E1A' }
                   elseif ($r.UsagePct -ge $WarningThresholdPct) { '#B05E00' }
                   else { '#0A7A09' }
            $rowCls = if ($r.UsagePct -ge $CriticalThresholdPct) { 'r-err' }
                      elseif ($r.UsagePct -ge $WarningThresholdPct) { 'r-warn' }
                      else { 'r-ok' }
            $progPct  = [math]::Min($r.UsagePct, 100)
            $progCell = "<div class='pw'><div class='pt'><div class='pf' style='width:${progPct}%;background:${col};'></div></div>" +
                        "<div class='pp' style='color:${col};'>$($r.UsagePct)%</div></div>"

            "<tr class='$rowCls'>" +
                "<td class='td-name'>$(HtmlEncode $r.Server)</td>" +
                "<td>$(HtmlEncode $r.OSVersion)</td>" +
        ...
        $TableRowsHtml = if ($RowsHtmlParts.Count -gt 0) {
            $RowsHtmlParts -join "`n                        "
        } else {
        ...
        $FinalHtml = $Html.
            Replace('{{GEN_DATE}}',        (HtmlEncode $GenDate)).
            Replace('{{SEC_HDR_LABEL}}',   $SecHdrLabel).
            Replace('{{TABLE_ROWS_HTML}}', $TableRowsHtml).
            Replace('{{FOOTER_HTML}}',     $FooterHtml)

        # Collection issues (if any) are kept as an HTML comment for
        # audit/troubleshooting only -- never shown on the rendered report.
        $AllErrors = @($Results | ForEach-Object { $_.Errors } | Where-Object { $_ })
        if ($AllErrors.Count -gt 0) {
            $ErrComment = "<!-- Collection issues ($($AllErrors.Count)):`n" +
                          (($AllErrors | ForEach-Object { " - $(HtmlEncode $_)" }) -join "`n") +
                          "`n-->`n</body>"
            $FinalHtml = $FinalHtml.Replace('</body>', $ErrComment)
        }
        ...
        # let the caller extract just the HTML between them. That HTML is now
        # fully self-contained (all data rendered server-side), so whatever
        # captures this stdout block -- 1 server or many -- gets one complete,
        # correct report with no separate render/merge step needed.
...
try {
    ...
    # Parse server list
    # Step 1: if HPSA/Camunda injected the value Base64-encoded, decode it first.
    # This is the recommended path -- it sidesteps command-line quoting entirely,
    # since Base64 text contains no ';', ',', or '"' for an unquoted/mis-quoted
    # command build to mangle. Falls through untouched if not actually Base64.
    $ResolvedServerFQDN = Resolve-PossibleBase64 -Value $LicenseServerFQDN

    # Step 2: split on ';' and/or ',' (both accepted for plain-text input/manual
    # testing). Comma is included because if the caller ever builds the command
    # line WITHOUT quoting the value, an unquoted ';' is treated by PowerShell as
    # a statement separator and servers after the first one never reach this
    # script at all (they get executed as bare, unrecognized commands instead).
    # Duplicate names (after trimming) are silently collapsed.
    $ServerList = @(
        $ResolvedServerFQDN -split '[;,]' |
    ...
    if ($ServerList.Count -eq 1) {
        ...
        try {
            ...
            if ($result.Installed -eq 0 -and ($result.OSVersion -eq 'N/A' -or $result.Errors.Count -gt 0)) {
                # No usable data came back (unreachable / no license role) --
                # keep this out of the HTML report, not a real "0% compliant" server.
                $FailedServers.Add($ServerList[0])
                Write-Log "No usable license data for [$($ServerList[0])] -- excluded from report." "ERROR"
            } else {
            ...
            foreach ($job in $Jobs) {
                try {
                    ...
                    if ($null -ne $result -and $result.Count -gt 0) {
                        $r = $result[0]
                        if ($r.Installed -eq 0 -and ($r.OSVersion -eq 'N/A' -or $r.Errors.Count -gt 0)) {
                            # No usable data came back -- exclude from the HTML report.
                            $FailedServers.Add($job.Server)
                            Write-Log "No usable license data for [$($job.Server)] -- excluded from report." "ERROR"
                        } else {
                            $ServerResults.Add($r)
                        }
                    } else {
