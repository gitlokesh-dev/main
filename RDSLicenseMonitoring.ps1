...
# ============================================================================
# ============================================================================
# SINGLE-SERVER REPORT OUTPUT
# Writes the one HTML file used when exactly one server was processed.
# Called once by the driver, never from inside the per-server loop.
# ============================================================================
...
function Invoke-LicenseServerReport {
    ...
    Write-Log "=== UC3 Complete for [$Server] | $Compliance ($UsagePct%) ==="
...
# Exactly one output file is produced per run: the per-server KPI report
# when one server was supplied, or the combined table when more than one
# server was supplied. Never both, and never per-server files for a batch.
