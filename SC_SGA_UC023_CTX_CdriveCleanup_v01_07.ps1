...
# No strict type on $Space — avoids OrderedDictionary type mismatch across PS versions
...
try {
    ...
    if (!(Test-Path -Path $acDir)) {
        ...
        $duration      = New-TimeSpan -Start $lastWriteTime -End (Get-Date)
        ...
        elseif (($runScore -eq 0) -and ($duration.TotalHours -ge $maxDuration)) {
            ...
            Write-Log "Automation control check passed. RunScore = $runScore" -Level INFO
        }
    }
}
catch {
    $errMsg = $_.Exception.Message
    $errMsg | Out-File -Append -FilePath "C:\Windows\Temp\UC023_AutoCtrl_Error.log" -ErrorAction SilentlyContinue
...
try {
    $profileCutoff = (Get-Date).AddDays(-$global:FileAgeDays)
        ...
        foreach ($sub in $userSubPaths) {
            ...
            try {
                Get-ChildItem -Path $target -File -Recurse -Force -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt $profileCutoff } |
                    ForEach-Object {
                        if ((New-TimeSpan -Start $global:ScriptStartTime -End (Get-Date)).TotalSeconds -ge $global:ScriptTimeoutSec) {
...
try {
    ...
    # Restore services even if cleanup failed
    Start-ServiceSafe -Name 'wuauserv'
    Start-ServiceSafe -Name 'TrustedInstaller'
}

# ─────────────────────────────────────────────
# PHASE 6: CleanMgr.exe (run only if present — no install attempt)
# ─────────────────────────────────────────────
...
if (-not (Test-Path $cleanMgrPath)) {
    ...
    try {
        ...
        # Launch with 120s hard timeout — never block the automation platform
